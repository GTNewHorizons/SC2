...
public class SlotLiquidInput extends SlotBase {
    ...
    public int getSlotStackLimit() {
        if (maxsize != -1) {
            ...
            /*
             * LiquidStack liquid = tank.getLiquid(); int space = tank.getCapacity(); if (liquid != null) { space -=
             * liquid.amount; } return Math.min(16, 1 + space / LiquidContainerRegistry.BUCKET_VOLUME);
             */
        }
    }
    ...
    public boolean isItemValid(ItemStack itemstack) {
        return (FluidContainerRegistry.isEmptyContainer(itemstack) && tank.getFluid() != null)
                || (FluidContainerRegistry.isFilledContainer(itemstack) && (tank.getFluid() == null
