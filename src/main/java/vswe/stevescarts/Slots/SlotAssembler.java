...
public class SlotAssembler extends Slot {
    ...
    public SlotAssembler(TileEntityCartAssembler assembler, int i, int j, int k, int groupID, boolean useLarge,
            int id) {
    ...
    public boolean isItemValid(ItemStack itemstack) {
        return itemstack != null && isValid
                && ModuleData.isValidModuleItem(groupID, itemstack)
                && (!getHasStack() || (getStack().stackSize > 0 && itemstack.stackSize > 0));
    }
    ...
    /*
     * public boolean isPartlyInvalid() { return !fullInvalidation && !isValid(); }
     */
