...
public abstract class CartVersion {
    ...
    static {
         ...
         * ------- THE LIQUID UPDATE -------
         * 
         * The Hydrators loses their liquid storages. Therefore the Large Hydrator will be replaced by the normal one.
         * Also, if a cart had a Hydrator that cart will receive a set of side tanks.
         **/
        new CartVersion() {
         ...
         * 
         * All tools has their durability set to 100%. Note that nothing is actually done here, this is because the
         * carts and items manages to handle everything pretty well. The only thing that has to be done is to move items
         * in some carts(since the Tool Modules now has an extra slot) and that is solved elsewhere (this is just a
