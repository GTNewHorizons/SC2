...
public class EnchantmentInfo {
    ...
    public static EnchantmentInfo efficiency = new EnchantmentInfo(
            Enchantment.efficiency,
            ENCHANTMENT_TYPE.TOOL,
            50000);
    public static EnchantmentInfo unbreaking = new EnchantmentInfo(
    ...
    public static EnchantmentData addBook(ArrayList<ENCHANTMENT_TYPE> enabledTypes, EnchantmentData data,
            ItemStack itemstack) {
        if (itemstack != null && itemstack.getItem() == Items.enchanted_book) {
            if (data == null) {
                for (EnchantmentInfo info : enchants) {
    ...
    private static EnchantmentData addEnchantment(ArrayList<ENCHANTMENT_TYPE> enabledTypes, EnchantmentData data,
            ItemStack itemstack, EnchantmentInfo info) {
        boolean isValid = false;
        for (ENCHANTMENT_TYPE type : enabledTypes) {
            if (info.type == type) {
                isValid = true;
