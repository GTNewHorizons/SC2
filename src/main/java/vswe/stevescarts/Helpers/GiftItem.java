...
public class GiftItem {
            ...
            if (module.getIsValid() && !module.getIsLocked() && module.getHasRecipe()) {
                if (module.getCost() > 0) {
                    GiftItem item = new GiftItem(
                            new ItemStack(ModItems.modules, 1, module.getID()),
                            module.getCost() * 20,
                            (int) Math.pow(151 - module.getCost(), 2));
                    item.fixedSize = true;
                    gifts.add(item);
                }
            }
    ...
    public static ArrayList<ItemStack> generateItems(java.util.Random rand, ArrayList<GiftItem> gifts, int value,
            int maxTries) {
