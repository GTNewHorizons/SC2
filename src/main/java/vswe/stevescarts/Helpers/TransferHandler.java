...
public class TransferHandler {
    ...
    public static void TransferItem(ItemStack iStack, IInventory inv, Container cont, java.lang.Class validSlot,
            int maxItems, TRANSFER_TYPE type) {
    ...
    public static void TransferItem(ItemStack iStack, IInventory inv, int start, int end, Container cont,
        ...
        // this loop will loop as long as there's more items to add to the inventory and it's still possible to add
        // those
        do {
            ...
            // first loop through the inventory once, this will try to find already used stacks where we could put the
            // items
            for (int i = startEmpty; i <= end; i++) {
                if (isSlotOfType(cont.getSlot(i), validSlot)
                    ...
                    // a super heavy if-statement. This will just compare the item in the slot and the item we want to
                    // store, if they are the same in various different properties they can be stored together.
                    if (inv.getStackInSlot(i) != null && inv.getStackInSlot(i).getItem() == iStack.getItem()
                            ...
                            && (inv.getStackInSlot(i).stackSize > 0 && iStack.stackSize > 0)
            ...
            if (pos != -1) {
                ...
                // first of all, if the slot is empty create an empty ItemStack there with our item. In this way we can
                // use the same code to add to an existing ItemStack and a new one.
                ...
                // now it's time to calculate how many items we can transfer. This is done by comparing the stack size
                // with all stack limits.
                ...
                // if there's no items to be moved there won't be any difference and therefore we have to exit(no
                // infinite loops here)
                if (stackSize <= 0) {
                    ...
                    // if the stack we want to store is empty we're done and therefore mark the slot position as invalid
                    // to exit the loop. If it isn't marked as invalid the loop will loop again to try to store the rest
                    // of the items.
