...
public class ModelShield extends ModelCartbase {
    ...
    public void applyEffects(ModuleBase module, float yaw, float pitch, float roll) {
        ...
        for (int i = 0; i < shields.length; i++) {
            for (int j = 0; j < shields[i].length; j++) {
                float a = shieldAngle
                        + (float) (Math.PI * 2) * (j / (float) shields[i].length + i / (float) shields.length);
                a = (float) (a % (Math.PI * 100));
                shieldAnchors[i][j].rotateAngleY = a;
                shields[i][j].rotationPointY = ((float) Math.sin(a / 5) * 3F + (i - (shields.length - 1) / 2F) * 5F
                        - 5F) * shieldDistance / 18F;
