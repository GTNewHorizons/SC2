...
public class GuiMinecart extends GuiBase {
    ...
    public void drawGuiBackground(float f, int x, int y) {
        ...
        if (thief != null) {
            ...
            drawTexturedModalRect(
                    j + scrollBox[0] + 2,
                    k + scrollBox[1] + 2 + cart.getScrollY(),
                    222 + 18,
                    26 + (cart.canScrollModules ? 0 : 25),
                    14,
                    25);
    ...
    private void renderModuleList(int x, int y) {
        ...
        for (int i = 0; i < moduleCounts.size(); i++) {
            ...
            this.drawIcon(
                    count.getData().getIcon(),
                    getGuiLeft() + getModuleDisplayX(i),
                    getGuiTop() + getModuleDisplayY(i),
                    1,
                    1,
                    0,
                    0);
    ...
    private void renderModuleListText(int x, int y) {
        ...
        for (int i = 0; i < moduleCounts.size(); i++) {
            ...
            if (count.getCount() != 1) {
                int alpha = (int) ((inRect(x, y, getModuleDisplayX(i), getModuleDisplayY(i), 16, 16) ? 1 : 0.75F)
                        * 256);

                String str = String.valueOf(count.getCount());
                getFontRenderer().drawStringWithShadow(
                        str,
                        getModuleDisplayX(i) + 16 - getFontRenderer().getStringWidth(str),
                        getModuleDisplayY(i) + 8,
                        0xFFFFFF | (alpha << 24));
            }
        }
        GL11.glDisable(GL11.GL_BLEND);
    ...
    private void drawModuleSlots(ModuleBase module) {
        if (module.hasGui()) {
            ...
            if (isDevelopmentEnvironment()) {
                ...
                drawTexturedModalRect(
                        getGuiLeft() + module.getX() + module.guiWidth() - 2,
                        getGuiTop() + module.getY(),
                        xSize - 256 + 3,
                        18,
                        3,
                        3);
                drawTexturedModalRect(
                        getGuiLeft() + module.getX() + module.guiWidth() - 2,
                        getGuiTop() + module.getY() + module.guiHeight() - 2,
                        xSize - 256 + 3,
                        18 + 3,
                        3,
                        3);
                drawTexturedModalRect(
                        getGuiLeft() + module.getX(),
                        getGuiTop() + module.getY() + module.guiHeight() - 2,
                        xSize - 256,
                        18 + 3,
                        3,
