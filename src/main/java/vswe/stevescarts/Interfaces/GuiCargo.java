...
public class GuiCargo extends GuiManager {
    ...
    private static ResourceLocation[] texturesLeft = { ResourceHelper.getResource("/gui/cargoVersion0Part1.png"),
            ResourceHelper.getResource("/gui/cargoVersion1Part1.png") };
    private static ResourceLocation[] texturesRight = { ResourceHelper.getResource("/gui/cargoVersion0Part2.png"),
            ResourceHelper.getResource("/gui/cargoVersion1Part2.png") };
    ...
    protected void drawItems(int id, RenderItem renderitem, int left, int top) {
        ...
        if (getCargo().target[id] < 0 || getCargo().target[id] >= getCargo().itemSelections.size()
                || getCargo().itemSelections.get(getCargo().target[id]).getIcon() == null) {
    ...
    protected void drawExtraOverlay(int id, int x, int y) {
        if (getCargo().target[id] >= 0 && getCargo().target[id] < getCargo().itemSelections.size()) {
            ...
            if (item.getName() != null) {
                drawMouseOver(
                        Localization.GUI.CARGO.CHANGE_STORAGE_AREA.translate() + "\n"
                ...
                drawMouseOver(
                        Localization.GUI.CARGO.CHANGE_STORAGE_AREA.translate() + "\n"
                                + Localization.GUI.MANAGER.CURRENT_SETTING.translate()
                                + ": "
                                + Localization.GUI.CARGO.UNKNOWN_AREA.translate(),
                        x,
                        y,
                        getBoxCoords(id));
            }
