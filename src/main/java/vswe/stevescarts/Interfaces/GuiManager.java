...
public abstract class GuiManager extends GuiBase {
    ...
    public void drawGuiForeground(int x, int y) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            drawMouseOver(
                    Localization.GUI.MANAGER.CHANGE_TRANSFER_DIRECTION.translate() + "\n"
                            + Localization.GUI.MANAGER.CURRENT_SETTING.translate()
                            + ": "
                            + (manager.toCart[i] ? Localization.GUI.MANAGER.DIRECTION_TO_CART.translate()
                                    : Localization.GUI.MANAGER.DIRECTION_FROM_CART.translate()),
                    x,
                    y,
                    getArrowCoords(i));
            drawMouseOver(
                    Localization.GUI.MANAGER.CHANGE_TURN_BACK_SETTING.translate() + "\n"
                            + Localization.GUI.MANAGER.CURRENT_SETTING.translate()
                            + ": "
                            + (manager.color[i] == 5 ? Localization.GUI.MANAGER.TURN_BACK_NOT_SELECTED.translate()
                                    : (manager.doReturn[manager.color[i] - 1]
                                            ? Localization.GUI.MANAGER.TURN_BACK_DO.translate()
                                            : Localization.GUI.MANAGER.TURN_BACK_DO_NOT.translate())),
                    x,
                    y,
                    getReturnCoords(i));
            drawMouseOver(
                    Localization.GUI.MANAGER.CHANGE_TRANSFER_SIZE.translate() + "\n"
                            + Localization.GUI.MANAGER.CURRENT_SETTING.translate()
                            + ": "
                            + getMaxSizeOverlay(i),
                    x,
            ...
            drawMouseOver(
                    Localization.GUI.MANAGER.CHANGE_SIDE.translate() + "\n"
                            + Localization.GUI.MANAGER.CURRENT_SIDE.translate()
                            + ": "
                            + (new String[] { Localization.GUI.MANAGER.SIDE_RED.translate(),
                                    Localization.GUI.MANAGER.SIDE_BLUE.translate(),
                                    Localization.GUI.MANAGER.SIDE_YELLOW.translate(),
                                    Localization.GUI.MANAGER.SIDE_GREEN.translate(),
                                    Localization.GUI.MANAGER.SIDE_DISABLED.translate() })[manager.color[i] - 1],
                    x,
                    y,
                    getColorpickerCoords(i));
        ...
        drawMouseOver(
                getLayoutString() + "\n"
                        + Localization.GUI.MANAGER.CURRENT_SETTING.translate()
                        + ": "
                        + getLayoutOption(manager.layoutType),
                x,
                y,
                getMiddleCoords());
    ...
    protected void drawColors(int id, int color, int left, int top) {
        ...
        drawTexturedModalRect(
                left + coords[0],
                top + coords[1],
                getColorSourceX() + (manager.doReturn[manager.color[id] - 1] ? 8 : 0),
                80 + 8 * color,
                8,
                8);
