...
public class GuiDistributor extends GuiBase {
        ...
        for (DistributorSide side : sides) {
            if (side.isEnabled(distributor)) {
                ...
                drawTexturedModalRect(
                        j + box[0] + 2,
                        k + box[1] + 2,
                        box[2] * 2 + (box[2] - 4) * side.getId(),
                        ySize,
                        box[2] - 4,
                        box[3] - 4);

                drawMouseMover(
                        Localization.GUI.DISTRIBUTOR.SIDE.translate(side.getName()) + (activeId != -1
                ...
                for (DistributorSetting setting : DistributorSetting.settings) {
                    if (setting.isEnabled(distributor)) {
                        if (side.isSet(setting.getId())) {
                            ...
                            drawMouseMover(
                                    setting.getName(invs) + "\n["
                                            + Localization.GUI.DISTRIBUTOR.REMOVE_INSTRUCTION.translate()
                                            + "]",
                                    x,
                                    y,
                                    settingbox);
    ...
    private void drawSetting(DistributorSetting setting, int[] box, boolean hover) {
        ...
        drawTexturedModalRect(
                j + box[0] + 1,
                k + box[1] + 1,
                box[2] * 4 + (box[2] - 2) * setting.getImageId(),
                ySize + getSideBoxRect(0)[3],
                box[2] - 2,
                box[3] - 2);
    }
    ...
    private int[] getActiveSettingBoxRect(int side, int setting) {
        ...
        return new int[] { sideCoords[0] + sideCoords[2] + 5 + setting * 18, sideCoords[1] + (sideCoords[3] - 16) / 2,
                16, 16 };
    }
                        ...
                        if (setting.isEnabled(distributor)) {
                            if (side.isSet(setting.getId())) {
                                ...
                                if (inRect(x, y, settingbox)) {
                                    distributor
                                            .sendPacket(1, new byte[] { (byte) setting.getId(), (byte) side.getId() });
                                }
