...
public class GuiCartAssembler extends GuiBase {
    ...
    private void updateErrorList() {
        ...
        if (assembler.getStackInSlot(0) == null) {
            ...
            if (hulldata == null || !(hulldata instanceof ModuleDataHull)) {
                ...
                addText(
                        lines,
                        Localization.GUI.ASSEMBLER.TOTAl_TIME.translate() + ": "
                                + formatTime((int) (assembler.generateAssemblingTime() / assembler.getEfficiency())));
                addNewLine(lines);
    ...
    public void drawGuiBackground(float f, int x, int y) {
        ...
        for (SlotAssembler slot : slots) {
            ...
            /*
             * if (slot.isPartlyInvalid()) { this.zLevel = 400; drawTexturedModalRect(j+targetX, k+targetY, srcX, size,
             * size, size); this.zLevel = 0; }
             */
        ...
        for (TitleBox box : assembler.getTitleBoxes()) {
            ...
            GL11.glColor4f(
                    (float) (box.getColor() >> 16) / 255.0F,
                    (float) (box.getColor() >> 8 & 255) / 255.0F,
                    (float) (box.getColor() & 255) / 255.0F,
                    1F);
        ...
        if (assembler.getIsAssembling()) {
            ...
            assemblingInfo = Localization.GUI.ASSEMBLER.ASSEMBLE_PROGRESS.translate() + ": "
                    + formatProgress(assemblingProgress);
            assemblingInfo += "\n" + Localization.GUI.ASSEMBLER.TIME_LEFT.translate()
                    + ": "
                    + formatTime(
                            (int) ((assembler.getMaxAssemblingTime() - assembler.getAssemblingTime())
                                    / assembler.getEfficiency()));

            // won't work, the client won't know about this
            /*
             * assemblingInfo += "\n\nModules:\n"; NBTTagCompound info = assembler.outputItem.getTagCompound(); if (info
             * != null) { NBTTagByteArray moduleIDTag = (NBTTagByteArray)info.getTag("Modules"); byte [] bytes =
             * moduleIDTag.byteArray; for (byte id : bytes) { ModuleData module = ModuleData.getList().get(id); if
             * (module != null) { assemblingInfo += module.getName() + "\n"; } } }
             */
        } else {
        ...
        drawProgressBarInfo(
                ...
                x,
                y,
                Localization.GUI.ASSEMBLER.FUEL_LEVEL.translate() + ": "
                        + assembler.getFuelLevel()
                        + "/"
                        + assembler.getMaxFuelLevel());
    }
    ...
    private void render3DCart() {
        ...
        GL11.glTranslatef(
                (float) (left + 512 / 2),
                (float) (top + (StevesCarts.instance.renderSteve ? 50 : 100)),
                100.0F);
        float scale = 50.0F;
        ...
        if (dropdownX != -1 && dropdownY != -1) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                drawTexturedModalRect(
                        j + rect[0] + 34,
                        k + rect[1] + 2,
                        (item.getImageID() % 16) * 16,
                        179 + (item.getImageID() / 16) * 16,
                        16,
                        16);
                        ...
                        if (item.getIsSubMenuOpen()) {
                            ...
                            for (int bool = 0; bool < count; bool++) {
                                drawBooleanBox(
                                        x,
                                        y,
                                        subrect[0] + getOffSetXForSubMenuBox(bool, count),
                                        subrect[1] + 3,
                                        item.getMULTIBOOL(bool));
                            }
                        }
