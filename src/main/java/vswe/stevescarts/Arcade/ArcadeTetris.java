...
public class ArcadeTetris extends ArcadeGame {
    ...
    public static final int BOARD_START_Y = (MinecartModular.MODULAR_SPACE_HEIGHT - 150) / 2;
    ...
    public void drawBackground(GuiMinecart gui, int x, int y) {
        ...
        if (!isPlaying) {
            ...
            getModule()
                    .drawImage(gui, BOARD_START_X, BOARD_START_Y + 150 - graphicalValue, 104, 40, 100, graphicalValue);

            if (graphicalValue == 150
                    && getModule().inRect(x, y, new int[] { BOARD_START_X, BOARD_START_Y, 100, 150 })) {
    ...
    public void drawForeground(GuiMinecart gui) {
        getModule()
                .drawString(gui, Localization.ARCADE.HIGH_SCORE.translate(String.valueOf(highscore)), 10, 20, 0x404040);
        getModule().drawString(gui, Localization.ARCADE.SCORE.translate(String.valueOf(score)), 10, 40, 0x404040);
        getModule().drawString(
                gui,
                Localization.ARCADE.REMOVED_LINES.translate(String.valueOf(removed)),
                10,
                60,
                0x404040);
        ...
        for (int i = 0; i < 4; i++) {
            getModule().drawString(
                    gui,
                    Localization.ARCADE.REMOVED_LINES_COMBO
                            .translate(String.valueOf(i), String.valueOf(removedByAmount[i])),
                    10,
                    80 + i * 10,
                    0x404040);
        }
