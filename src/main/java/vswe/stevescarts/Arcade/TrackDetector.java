...
public class TrackDetector extends Track {
    ...
    public void setExtraInfo(byte[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (startPosition == 0) {
                ...
                content = (short) ((val & (((int) Math.pow(2, (startPosition - 1)) - 1) << (9 - startPosition)))
                        >> (9 - startPosition));
    ...
    public byte[] getExtraInfo() {
        ...
        int startPosition = 0;
        for (int i = 0; i < targets.size(); i++) {
            short data = (short) targets.get(i).getX(); // 5 bits
            data |= targets.get(i).getY() << 5; // 4 bits

            ret[currentByte] |= (byte) ((data & ((int) Math.pow(2, 8 - startPosition) - 1)) << startPosition);
            currentByte++;
            ret[currentByte] = (byte) ((data & (((int) Math.pow(2, 1 + startPosition) - 1) << (8 - startPosition)))
                    >> (8 - startPosition));
