...
public abstract class TileEntityManager extends TileEntityBase implements IInventory {
    ...
    public boolean exchangeItems(ManagerTransfer transfer) {
        ...
        for (transfer.setSetting(transfer.getLowestSetting()); transfer.getSetting() < 4; transfer
                .setSetting(transfer.getSetting() + 1)) {
            ...
            if (!(toCart[transfer.getSetting()] ? transfer.getToCartEnabled() : transfer.getFromCartEnabled())
                    || !isTargetValid(transfer)) {
