...
public class TileEntityLiquid extends TileEntityManager implements IFluidHandler, ITankHolder, ISidedInventory {
    ...
    public void drawImage(int tankid, GuiBase gui, IIcon icon, int targetX, int targetY, int srcX, int srcY, int sizeX,
        ...
        gui.drawIcon(
                icon,
                gui.getGuiLeft() + targetX,
                gui.getGuiTop() + targetY,
                sizeX / 16F,
                sizeY / 16F,
                srcX / 16F,
                srcY / 16F);
    ...
    protected boolean doTransfer(ManagerTransfer transfer) {
        int maximumToTransfer = hasMaxAmount(transfer.getSetting()) ? Math
                .min(getMaxAmount(transfer.getSetting()) - transfer.getWorkload(), FluidContainerRegistry.BUCKET_VOLUME)
        ...
        if (sucess && hasMaxAmount(transfer.getSetting())
                && transfer.getWorkload() == getMaxAmount(transfer.getSetting())) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if ((isNew || con.oldLiquids[i] != null) && tanks[i].getFluid() == null) {
                ...
                if (isNew || con.oldLiquids[i] == null) {
                    updateGuiData(con, crafting, id, (short) tanks[i].getFluid().getFluidID());
                    ...
                    if (con.oldLiquids[i].getFluidID() != tanks[i].getFluid().getFluidID()) {
                        updateGuiData(con, crafting, id, (short) tanks[i].getFluid().getFluidID());
                        changed = true;
                    }
    ...
    public void receiveGuiData(int id, short data) {
        if (id > 3) {
            ...
            if (contentid == 0) {
                ...
                tanks[tankid]
                        .getFluid().amount = getIntFromShort(contentid == 1, tanks[tankid].getFluid().amount, data);
