...
public class RendererMinecart extends Render {
    ...
    public void renderCart(MinecartModular cart, double x, double y, double z, float yaw, float partialTickTime) {
        ...
        float partialRotPitch = cart.prevRotationPitch
                + (cart.rotationPitch - cart.prevRotationPitch) * partialTickTime;

        /*
         * Vec3 rotations =
         * engine.getRenderRotation((float)partialPosX,(float)partialPosY,(float)partialPosZ,partialTickTime); if
         * (rotations != null) { yaw = (float)rotations.xCoord * 180F / (float)Math.PI; }
         */
    ...
    public void renderLiquidCuboid(FluidStack liquid, int tankSize, float x, float y, float z, float sizeX, float sizeY,
            float sizeZ, float mult) {
    ...
    private void renderFace(IIcon icon, double totalTargetW, double totalTargetH, float yaw, float roll, float offX,
        ...
        while (totalTargetW - currentTargetX > d * 2) {
            ...
            while (totalTargetH - currentTargetY > d * 2) {
                ...
                tessellator.addVertexWithUV(
                        currentTargetX - totalTargetW / 2,
                        currentTargetY - totalTargetH / 2,
                        0D,
                        srcX,
                        srcY);
                tessellator.addVertexWithUV(
                        currentTargetX + currentTargetW - totalTargetW / 2,
                        currentTargetY - totalTargetH / 2,
                        0D,
                        srcX + srcW * currentTargetW,
                        srcY);
                tessellator.addVertexWithUV(
                        currentTargetX + currentTargetW - totalTargetW / 2,
                        currentTargetY + currentTargetH - totalTargetH / 2,
                        0D,
                        srcX + srcW * currentTargetW,
                        srcY + srcH * currentTargetH);
                tessellator.addVertexWithUV(
                        currentTargetX - totalTargetW / 2,
                        currentTargetY + currentTargetH - totalTargetH / 2,
                        0D,
