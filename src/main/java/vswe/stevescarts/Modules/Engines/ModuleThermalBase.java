...
public abstract class ModuleThermalBase extends ModuleEngine {
    ...
    public int getTotalFuel() {
        int totalfuel = getFuelLevel()
                + getCart().drain(FluidRegistry.LAVA, Integer.MAX_VALUE, false) * getEfficiency();

        if (requiresCoolant()) {
            int totalcoolant = getCoolantLevel()
                    + getCart().drain(FluidRegistry.WATER, Integer.MAX_VALUE, false) * getCoolantEfficiency();
    ...
    public void smoke() {
        /*
         * double oX = 0.0; double oZ = 0.0; if (getCart().motionX != 0) { oX = (getCart().motionX > 0 ? -1 : 1); } if
         * (getCart().motionZ != 0) { oZ = (getCart().motionZ > 0 ? -1 : 1); } if (getCart().rand.nextInt(2) == 0) {
         * getCart().worldObj.spawnParticle("largesmoke", getCart().posX + oX * 0.85, getCart().posY + 0.12D,
         * getCart().posZ + oZ * 0.85, 0.0D, 0.0D, 0.0D); }
         */
    }
