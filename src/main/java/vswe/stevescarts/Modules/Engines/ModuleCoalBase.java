        ...
        if (getFuelLevel() <= consumption) {
            ...
            for (int i = 0; i < getInventorySize(); i++) {
                ...
                if (getFuelLevel() > consumption) {
                     ...
                     * getCart().pushX = getCart().temppushX; getCart().pushZ = getCart().temppushZ; }
                     */
    ...
    public void smoke() {
        ...
        if (getCart().rand.nextInt(2) == 0) {
            getCart().worldObj.spawnParticle(
                    "largesmoke",
                    getCart().posX + oX * 0.85,
                    getCart().posY + 0.12D,
                    getCart().posZ + oZ * 0.85,
                    0.0D,
                    0.0D,
                    0.0D);
        }
        ...
        if (getCart().rand.nextInt(30) == 0) {
            ...
            // getCart().worldObj.spawnParticle("flame",getCart().posX + oX* 0.75, getCart().posY + 0.15D,
            // getCart().posZ + oZ*0.75, 0, 0,0);
            // }else{
            getCart().worldObj.spawnParticle(
                    "flame",
                    getCart().posX + oX * 0.75,
                    getCart().posY + 0.15D,
                    getCart().posZ + oZ * 0.75,
                    getCart().motionX,
                    getCart().motionY,
                    getCart().motionZ);
            // }
