...
public abstract /* remove the abstract */ class ModuleCommand extends ModuleBase implements ICommandSender {
    ...
    private void executeCommand() {
        if (!getCart().worldObj.isRemote) {
            /*
             * MinecraftServer server = MinecraftServer.getServer(); if (server != null && server.func_82356_Z()) {
             * ICommandManager commandManager = server.getCommandManager(); commandManager.executeCommand(this,
             * command); }
             */
        }
    }
