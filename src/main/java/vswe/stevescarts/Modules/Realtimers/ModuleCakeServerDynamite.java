...
public class ModuleCakeServerDynamite extends ModuleCakeServer {
    ...
    private void explode() {
        getCart().worldObj
                .createExplosion(null, getCart().posX, getCart().posY, getCart().posZ, dynamiteCount * 0.8F, true);
    }
    ...
    public void update() {
        ...
        if (!getCart().worldObj.isRemote) {
            ...
            if (item != null && item.getItem().equals(ModItems.component)
                    && item.getItemDamage() == 6
                    && dynamiteCount < getMaxDynamiteCount()) {
