...
public class ModuleDynamite extends ModuleBase {
    ...
    private int[] getMovableMarker() {
        return new int[] { fuseStartX + (int) (105 * (1F - getFuseLength() / (float) maxFuseLength)), fuseStartY, 4,
                10 };
    }
    ...
    public void drawBackground(GuiMinecart gui, int x, int y) {
        ...
        drawImage(
                gui,
                fuseStartX + (int) (105 * (1F - (getFuseLength() - getFuse()) / (float) maxFuseLength)),
                fuseStartY,
                (isPrimed() ? 8 : 4),
                0,
                4,
    ...
    private void explode() {
        if (isPlaceholder()) {
            ...
            getCart().worldObj.createExplosion(
                    null,
                    getCart().posX,
                    getCart().posY,
                    getCart().posZ,
                    f,
                    true/* true means huge, false otherwise */);
        }
    }
    ...
    public void createExplosives() {
        ...
        /*
         * if (getStack(0) != null && getStack(0).getItem().itemID == Item.gunpowder.itemID) { f +=
         * getStack(0).stackSize * 1; } else if (getStack(0) != null && getStack(0).getItem().itemID ==
         * Block.tnt.blockID) { f += getStack(0).stackSize * 5; }
         */
