...
public class ModuleAdvControl extends ModuleBase implements ILeverModule {
    ...
    public void renderOverlay(net.minecraft.client.Minecraft minecraft) {
        ...
        drawImage(
                5 + 9,
                enginesEndAt + 13 + 12 - speedGraphicHeight,
                32 + 9,
                15 + 13 + 12 - speedGraphicHeight,
                14,
                speedGraphicHeight);
        ...
        minecraft.fontRenderer
                .drawString(Localization.MODULES.ATTACHMENTS.ODO.translate(), 5 + 2, enginesEndAt + 52 + 2, 0x404040);
        minecraft.fontRenderer.drawString(distToString(odo), 5 + 2, enginesEndAt + 52 + 11, 0x404040);
        minecraft.fontRenderer
                .drawString(Localization.MODULES.ATTACHMENTS.TRIP.translate(), 5 + 2, enginesEndAt + 52 + 22, 0x404040);
    ...
    public void update() {
        ...
        if (!getCart().worldObj.isRemote && getCart().riddenByEntity != null
                && getCart().riddenByEntity instanceof EntityPlayer) {
    ...
    // the reason prePosX etc. isn't used is to make sure that the calculation takes place exactly the same number of
    // times the last values are saved.
    ...
    private void encodeKeys() {
        if (getCart().riddenByEntity != null && getCart().riddenByEntity instanceof EntityPlayer
                && getCart().riddenByEntity == getClientPlayer()) {
