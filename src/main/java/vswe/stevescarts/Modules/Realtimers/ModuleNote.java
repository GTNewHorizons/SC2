...
public class ModuleNote extends ModuleBase {
    ...
    private String[] pitchNames = new String[] { "F#", "G", "G#", "A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#",
            "G", "G#", "A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#" };
    private Localization.MODULES.ATTACHMENTS[] instrumentNames = new Localization.MODULES.ATTACHMENTS[] {
            Localization.MODULES.ATTACHMENTS.PIANO, Localization.MODULES.ATTACHMENTS.BASS_DRUM,
            Localization.MODULES.ATTACHMENTS.SNARE_DRUM, Localization.MODULES.ATTACHMENTS.STICKS,
            Localization.MODULES.ATTACHMENTS.BASS_GUITAR };
    ...
    public ModuleNote(MinecartModular cart) {
        ...
        if (getCart().worldObj.isRemote) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (i > 0) {
                    tempButton.text = Localization.MODULES.ATTACHMENTS.ACTIVATE_INSTRUMENT
                            .translate(instrumentNames[i - 1].translate());
                } else {
        ...
        public void play(float volume) {
            ...
            if (!getCart().worldObj.isRemote) {
                if (volume > 0) {
                    ...
                    getCart().worldObj.playSoundEffect(
                            (double) getCart().x() + 0.5D,
                            (double) getCart().y() + 0.5D,
                            (double) getCart().z() + 0.5D,
                            "note." + instrumentString,
                            volume,
                            calculatedPitch);
                }
            } else {
                ...
                getCart().worldObj.spawnParticle(
                        "note",
                        (double) getCart().x() + oZ * 1 + 0.5D,
                        (double) getCart().y() + 1.2D,
                        (double) getCart().z() + oX * 1 + 0.5D,
                        (double) pitch / 24.0D,
                        0.0D,
                        0.0D);
                getCart().worldObj.spawnParticle(
        ...
        for (int i = getScrollY(); i < Math.min(tracks.size(), getScrollY() + tracksInView); i++) {
            ...
            for (int j = getScrollX(); j < Math.min(track.notes.size(), getScrollX() + notesInView); j++) {
                ...
                if (note.instrumentId != 0) {
                    drawStringOnMouseOver(
                            gui,
                            note.toString(),
                            x,
                            y,
                            note.getBounds(i - getScrollY(), j - getScrollX()));
                }
            }
        }
    ...
    protected void receivePacket(int id, byte[] data, EntityPlayer player) {
        if (id == 0) {
            ...
            byte instrumentInfo = (byte) (((byte) (info & ~(byte) maximumTracksPerModule))
                    >> (byte) maximumTracksPerModuleBitCount);
