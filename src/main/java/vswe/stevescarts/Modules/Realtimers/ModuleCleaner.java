...
public class ModuleCleaner extends ModuleBase {
    ...
    private void clean() {
        List list = getCart().worldObj
                .getEntitiesWithinAABBExcludingEntity(getCart(), getCart().boundingBox.expand(1D, 0.5D, 1D));
        ...
        for (int e = 0; e < list.size(); e++) {
            if (list.get(e) instanceof EntityItem) {
                ...
                if (eItem.delayBeforeCanPickup <= 10 && !eItem.isDead) {
                    ...
                    if (stackSize != eItem.getEntityItem().stackSize) {
                        getCart().worldObj.playSoundAtEntity(
                                ...
                                "random.pop",
                                0.2F,
                                ((getCart().rand.nextFloat() - getCart().rand.nextFloat()) * 0.7F + 1.0F) * 2.0F);
                ...
                if (Math.pow(eItem.motionX, 2) + Math.pow(eItem.motionY, 2) + Math.pow(eItem.motionZ, 2) < 0.2D
                        && eItem.arrowShake <= 0
                        && !eItem.isDead) {
                    eItem.arrowShake = 3;
