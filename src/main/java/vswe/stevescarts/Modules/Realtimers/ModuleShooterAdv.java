...
public class ModuleShooterAdv extends ModuleShooter {
    ...
    protected void Shoot() {
        ...
        if (target != null) {
            if (hasProjectileItem()) {
                ...
                getCart().worldObj
                        .playAuxSFX(1001, (int) getCart().posX, (int) getCart().posY, (int) getCart().posZ, 0);
            }
        }
    }
    ...
    private void shootAtTarget(Entity target) {
        ...
        if (dis >= 1.0E-7D) {
            ...
            projectile.setLocationAndAngles(
                    getCart().posX + disPX * 1.5F,
                    projectile.posY,
                    getCart().posZ + disPZ * 1.5,
                    theta,
                    phi);
        ...
        getCart().worldObj
                .playSoundAtEntity(getCart(), "random.bow", 1.0F, 1.0F / (getCart().rand.nextFloat() * 0.4F + 0.8F));
    ...
    private Entity getTarget() {
        List entities = getCart().worldObj.getEntitiesWithinAABB(
                Entity.class,
                getCart().boundingBox.expand((double) getTargetDistance(), 4.0D, (double) getTargetDistance()));
    ...
    private boolean canSee(Entity target) {
        ...
        return getCart().worldObj.rayTraceBlocks(
                Vec3.createVectorHelper(
                        getCart().posX,
                        getCart().posY + (double) getCart().getEyeHeight(),
                        getCart().posZ),
