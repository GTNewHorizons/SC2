...
public class ModuleRemover extends ModuleWorker {
    ...
    private boolean isRailAtCoords(Vec3 coords) {
        ...
        return BlockRailBase.func_150049_b_(getCart().worldObj, x, y + 1, z)
                || BlockRailBase.func_150049_b_(getCart().worldObj, x, y, z)
    ...
    private boolean removeRail(int x, int y, int z, boolean flag) {
        if (flag) {
            if (BlockRailBase.func_150049_b_(getCart().worldObj, x, y, z)
                    && getCart().worldObj.getBlock(x, y, z) == Blocks.rail) {
                if (!doPreWork()) {
                    ItemStack iStack;
                    int rInt = getCart().rand.nextInt(100);

                    /*
                     * if (rInt <= 1) { iStack = new ItemStack(StevesCarts.component, 1, 14); } else if (rInt <= 5) {
                     * iStack = new ItemStack(StevesCarts.component, 1, 15); } else if (rInt <= 11) { iStack = new
                     * ItemStack(StevesCarts.component, 1, 16); } else {
                     */
