...
public abstract class ModuleWorker extends ModuleBase {
    ...
    private Vec3 getNextblock(boolean flag) {
        ...
        // if there's a rail block below the cart, decrease the j value since the cart should therefore be counted as
        // being on that rail
        ...
        if (BlockRailBase.func_150051_a(b)) {
            ...
            // if it is for both x and z value then the cart is moving along the first direction(index 0) otherwise it's
            // moving along the second direction(index 1).
            int dir = ((xDir && zDir) == flag) ? 0 : 1;

            // return a vector with the coordinates of where the cart is heading
            return Vec3.createVectorHelper(i + logic[dir][0], j + logic[dir][1], k + logic[dir][2]);
        } else {
    ...
    protected boolean isValidForTrack(int i, int j, int k, boolean flag) {
        boolean result = countsAsAir(i, j, k)
                && (!flag || World.doesBlockHaveSolidTopSurface(getCart().worldObj, i, j - 1, k));
