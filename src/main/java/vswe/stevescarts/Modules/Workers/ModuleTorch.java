        ...
        if (light <= lightLimit) {
            ...
            for (int side = -1; side <= 1; side += 2) {
                ...
                for (int level = 2; level >= -2; level--) {
                    ...
                    if (getCart().worldObj.isAirBlock(xTorch, y + level, zTorch)
                            && Blocks.torch.canPlaceBlockAt(getCart().worldObj, xTorch, y + level, zTorch)) {
                        ...
                        // if it isn't valid but there's already a torch there this side is already done. This shouldn't
                        // really happen since then it wouldn't be dark enough in the first place.
