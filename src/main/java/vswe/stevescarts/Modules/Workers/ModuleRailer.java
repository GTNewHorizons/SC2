...
public class ModuleRailer extends ModuleWorker implements ISuppliesModule {
    ...
    protected ArrayList<Integer[]> getValidRailPositions(int x, int y, int z) {
        ...
        /*
         * if (!BlockRail.isRailBlockAt(getCart().worldObj, x, y, z)) { lst.add(new Integer[] {x + 1, y , z}); }
         */
    ...
    private boolean tryPlaceTrack(int i, int j, int k, boolean flag) {
        ...
        if (isValidForTrack(i, j, k, true)) {
            ...
            for (int l = 0; l < getInventorySize(); l++) {
                ...
                if (getStack(l) != null) {
                    if (validRail(getStack(l).getItem())) {
                        // if so this is a valid action to do, if the flag parameter is true this action should also be
                        // done
                        if (flag) {
                            // place the rail
            ...
            // if there's no rails left, return true so it won't test other blocks(there won't be any rails no matter
            // which block it tries to build on). Also tell the cart to turn back.
            turnback();
            return true;
        }
    ...
    /*
     * @Override public void getModels(HashMap<String,modMCBase> models) { for (baseModule module :
     * getCart().getModules()) { ModuleData data = ModuleData.getList().get(module.getModuleId()); if (data != null &&
     * data.getRenderingSides() != null) { if (data.getRenderingSides().contains(ModuleData.SIDE.TOP)) { return; } } } }
     */
