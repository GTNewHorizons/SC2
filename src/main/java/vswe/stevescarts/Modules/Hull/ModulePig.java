...
public class ModulePig extends ModuleHull {
    ...
    private void oink() {
        getCart().worldObj.playSoundAtEntity(
                getCart(),
                "mob.pig.say",
                1.0F,
                (getCart().rand.nextFloat() - getCart().rand.nextFloat()) * 0.2F + 1.0F);
    }
    ...
    public ResourceLocation getHelmetResource(boolean isOverlay) {
        if (hasHelment()) {
            ...
            return RenderBiped
                    .getArmorResource((AbstractClientPlayer) null, item, 0, isOverlay ? "overlay" : (String) null);

            // old code left here for reference
            /*
             * if (item.getItem() instanceof IArmorTextureProvider) { return
             * ((IArmorTextureProvider)item.getItem()).getArmorTextureFile(item); }else{ int index =
             * ((ItemArmor)item.getItem()).renderIndex; if (index < 0 || index >=
             * RenderPlayer.armorFilenamePrefix.length) { return null; } if (isOverlay) { return "/armor/" +
             * RenderPlayer.armorFilenamePrefix[index] + "_1_b.png"; }else{ return "/armor/" +
             * RenderPlayer.armorFilenamePrefix[index] + "_1.png"; } }
             */
