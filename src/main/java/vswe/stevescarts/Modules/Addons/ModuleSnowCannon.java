        ...
        for (int x = -getBlocksOnSide(); x <= getBlocksOnSide(); x++) {
            for (int z = -getBlocksOnSide(); z <= getBlocksOnSide(); z++) {
                for (int y = -getBlocksFromLevel(); y <= getBlocksFromLevel(); y++) {
                    ...
                    int y1 = getCart().y() + y;
                    int z1 = getCart().z() + z;
                    if (countsAsAir(x1, y1, z1)
                            && getCart().worldObj.getBiomeGenForCoords(x1, z1).getFloatTemperature(x1, y1, z1)
                                    <= 1.0F /* snow golems won't be hurt */
                            && Blocks.snow.canPlaceBlockAt(getCart().worldObj, x1, y1, z1)) {
