            ...
            for (int i = 0; i < 3; i++) {
                if (getStack(i) != null && getStack(i).stackSize > 0) {
                    ...
                    if (getStack(i).stackSize != stacksize) {
                        ...
                        for (int j = 0; j < 3; j++) {
                            if (i != j) {
                                if (enchants[i] != null && enchants[j] != null
                                        && enchants[i].getEnchantment() == enchants[j].getEnchantment()) {
                                    enchants[i] = null;
        ...
        for (int i = 0; i < 3; i++) {
            if (enchants[i] == null) {
                ...
                tagCompound.setShort(
                        generateNBTName("EffectId" + i, id),
                        (short) enchants[i].getEnchantment().getEnchantment().effectId);
