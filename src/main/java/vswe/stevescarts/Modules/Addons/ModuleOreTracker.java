...
public class ModuleOreTracker extends ModuleAddon {
    ...
    private BlockCoord findBlockToMine(ModuleDrill drill, ArrayList<BlockCoord> checked, BlockCoord current,
            boolean first) {
                ...
                for (int y = -1; y <= 1; y++) {
                    for (int z = -1; z <= 1; z++) {
                        if (Math.abs(x) + Math.abs(y) + Math.abs(z) == 1) {
                            BlockCoord ret = findBlockToMine(
                                    drill,
                                    checked,
                                    new BlockCoord(current.getX() + x, current.getY() + y, current.getZ() + z),
                                    false);
