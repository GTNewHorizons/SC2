...
public class ModuleLiquidSensors extends ModuleAddon {
    ...
    public boolean isDangerous(ModuleDrill drill, int x, int y, int z, int p, int q, int r) {
        ...
        // 2.1. the liquid is at the side but has flown so far that it can't spread further -> nothing will happen ->
        // it's alright
        ...
        // 2.3. the liquid is at the side but has flown so far that it can only spread ONE block more -> might be
        // alright(see below)
        // 2.3.1. the liquid is at the bottom of the tunnel -> it will spread one block -> the cart is too far away ->
        // it's alright
        // 2.3.2. the liquid is not at the bottom -> it will spread one block and then fall -> it will flow to the
        // bottom and start to spread -> the cart will be in the way -> not good
        // 2.4. none of the above -> the liquid will flow and destroy the cart -> not good
        // 3. when the block is removed sand or gravel will fall down -> liquid on top of this will fall down -> the
        // liquid will hit the cart -> not good (this is very difficult to detect(maybe not :P))
        ...
        if (isLiquid) {
            ...
            if (q == 1) {
                ...
                /*
                 * else if (isOther && ((m & 7) == ((IFluidBlock)block).getFlowDistance())) { return false; //case 2.1.
                 * }
                 */
                else {
                    handleLiquid(drill, x1, y1, z1);
            ...
            if (q == 1) {
                ...
                if (isFalling) {
                    return isDangerous(drill, x1, y1, z1, 0, +1, 0) || isDangerous(drill, x1, y1, z1, +1, 0, 0)
