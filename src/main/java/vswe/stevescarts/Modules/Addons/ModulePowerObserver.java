        ...
        for (int i = 0; i < getCart().getEngines().size(); i++) {
            if (!removeOnPickup() || currentEngine != i) {
                ...
                drawStringOnMouseOver(
                        gui,
                        engine.getData().getName() + "\n"
                                + Localization.MODULES.ADDONS.OBSERVER_INSTRUCTION.translate(),
                        x,
                        y,
                        getEngineRect(i));
            }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < getCart().getEngines().size(); j++) {
                if ((areaData[i] & (1 << j)) != 0) {
                    ...
                    drawStringOnMouseOver(
                            gui,
                            engine.getData().getName() + "\n" + Localization.MODULES.ADDONS.OBSERVER_REMOVE.translate(),
                            x,
                            y,
                            getEngineRectInArea(i, count));
                    count++;
                }
            }
            ...
            drawStringOnMouseOver(
                    gui,
                    Localization.MODULES.ADDONS.OBSERVER_CHANGE.translate() + "\n"
                            + Localization.MODULES.ADDONS.OBSERVER_CHANGE_10.translate(),
                    x,
                    y,
                    getPowerRect(i));
        }
